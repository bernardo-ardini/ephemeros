...
h=0.1; % volendo anche h=0.05 funziona, mentre con h più piccolo non più
...
Omega=geo.whole;
GammaD=Omega.bound.sub(@(x) 0*(abs(abs(x(2,:))-H)<geo.tol));
GammaDn=Omega.bound.sub(@(x) (abs(x(2,:)+H)<geo.tol)|(abs(x(1,:)+L)<geo.tol)|((abs(x(2,:)-H)<geo.tol)&(x(1,:)>0)));
...
md.dat.g=@(x,t) -2e-2*0.5*(1-cos(pi*t/md.T))*[0*x(2,:);((abs(x(2,:)-H)<geo.tol))];
