...
GammaDn=Omega.bound.sub(@(x) (abs(x(1,:))<geo.tol)|((abs(x(1,:)-75)<geo.tol)));
...
gamma=7;
md.dat.f=@(x,t) 0.5*(1-cos(pi*t/md.T))*[0;-4*gamma]*(x(1,:)>-10);
md.dat.g=@(x,t) 0*(1-cos(pi*t/md.T))*[0*x(2,:);((abs(x(2,:)-1)<geo.tol))];
...
md.mat.C=160.85906*I+2*90.71141*(Id+Tr);
eta=6;
