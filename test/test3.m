...
Omega=geo.whole;
GammaD=Omega.bound.sub(@(x) abs(x(2,:)+H)<geo.tol);
...
pl.setting.step.dtmin=1e-5;
pl.setting.step.dt0=0.01;
pl.setting.step.nmin=20;
pl.setting.step.nmax=3;

% newton-raphson setting

pl.setting.newton.tol=1e-7;
pl.setting.newton.maxiter=5;
pl.setting.step.maxgood=4;

% data

pl.dat.f=@(x,t) repmat([-0.01*(1-cos(2*pi*t/10));-(1-cos(2*pi*t/10))],[1,size(x,2)]);
pl.dat.g=@(x,t) repmat([0;0],[1,size(x,2)]);
pl.unit.strain=0.1/400;
